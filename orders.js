...
//Загрузка блюд с указанного апи
function loadDishes() {
...
//Загрузка блюд из заказа с отправкой hhtp-запроса
function loadOrders() {
    return fetch('https://edu.std-900.ist.mospolytech.ru/labs/api/orders?api_key=53c010a5-dd23-448e-ba0f-9df5fee7e3e3', {
            method: 'GET'
    })
        .then(response => response.json())
        .then(data => {
...
function editOrder(dishes, order) {
    ...
    document.getElementById('saveButton').addEventListener('click', () => {
        ...
        fetch(`https://edu.std-900.ist.mospolytech.ru/labs/api/orders/${order['id']}?api_key=53c010a5-dd23-448e-ba0f-9df5fee7e3e3`, {
            method: 'PUT',
            body: formData,
        })
            .then((response) => response.json())
